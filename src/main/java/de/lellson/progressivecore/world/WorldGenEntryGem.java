...
public class WorldGenEntryGem extends WorldGenEntry {
	...
	public WorldGenEntryGem(String name, Block block, GenType type, int minY, int maxY, int spawnChance) {
	...
	public void generate(World world, int chunkX, int chunkZ) {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            if (state.getBlock().isReplaceableOreGen(state, world, blockpos, net.minecraft.block.state.pattern.BlockMatcher.forBlock(type.getReplaceBlock())))
            {
	...
	protected IBlockState getState(World world, int chunkX, int chunkZ) {
		return ((BlockOreGem)block).getStateFromMeta(BlockOreGem.getMetaForBiome(getBiome(world, chunkX, chunkZ)));
	}
}
